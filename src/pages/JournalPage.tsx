...
const posts = [
  ...
  { id: '2', title: 'Возраст как освобождение', category: 'Возраст', excerpt: 'Когда ты перестаёшь оправдываться. И начинаешь жить.', date: '2025-02-03' },
  ...
  { id: '5', title: 'Позднее материнство — не приговор', category: 'Материнство', excerpt: 'И каждый ребёнок стал чудом.', date: '2025-03-25' },
        ...
        <div className="max-w-[1400px] mx-auto">
          <ScrollReveal>
            ...
            <p className="body-editorial text-muted-foreground max-w-xl mb-16">
              Тексты о женском пути, переменах и праве быть собой.
            </p>
          </ScrollReveal>
            ...
            <div className="flex flex-wrap gap-0 mb-16">
              {categories.map(cat => (
                <button
                  ...
                  className={`px-6 py-3 text-xs font-bold uppercase tracking-widest border-[3px] -ml-[3px] first:ml-0 transition-all duration-200 ${filter === cat ? 'border-primary bg-primary text-primary-foreground' : 'border-foreground/10 text-muted-foreground hover:text-foreground hover:border-primary/50'}`}
                >
                  {cat}
                </button>
              ))}
            ...
            {filtered.map((post, i) => (
              <ScrollReveal key={post.id} delay={i * 60}>
                <button
                  ...
                  className="w-full text-left p-8 border-[3px] border-foreground/10 -mt-[3px] hover:border-primary hover:bg-primary/5 transition-all duration-200 group active:translate-x-[2px] active:translate-y-[2px]"
                >
